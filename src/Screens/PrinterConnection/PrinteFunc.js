...
export const printReciept = async data => {
  ...
  return new Promise((resolve, reject) => {
    ...
    BluetoothManager.checkBluetoothEnabled().then(
      async enabled => {
        let total_amt_result = 0;
        let total_amt;
        // let addonPrice = 0;
        await detail.vendors[0].products.forEach(async el => {
          total_amt_result += Number(el.quantity * el.price);
        });
        total_amt = Number(total_amt_result).toFixed(2);
        ...
        if (enabled && isConnected) {
          ...
          try {
            ...
            await BluetoothEscposPrinter.printColumn(
              ...
              [`Wallet`, `-${detail.wallet_amount_used.toString()}` + '\r\n'],
              {},
            );
            await BluetoothEscposPrinter.printColumn(
              [15, 30],
...
export const printRecieptWithSunmi = async (data, _data) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    // Loop through the data array and sum up the quantity_price

    // let result = Number(el.quantity * el.price + totalQuantityPrice).toFixed(2);
    await detail.vendors[0].products.forEach(async el => {
      total_amt_result += Number(el.quantity * el.price);
      el?.addon.forEach(itm => {
        addonPrice += Number(itm.option?.price * el.quantity);
      });
    });
    total_amt = Number(total_amt_result + addonPrice).toFixed(2);
    try {
      const base64Data = await getBase64Image(
        `${detail.admin_profile.logo.image_fit}200/200${detail.admin_profile.logo.image_path}`,
      );
      ...
      detail.vendors[0].products.forEach(async el => {
        ...
        // Loop through the data array and sum up the quantity_price
        el?.addon.forEach(itm => {
          totalQuantityPrice += Number(itm.option?.price * el.quantity);
        });
        let result = Number(
          el.quantity * el.price + totalQuantityPrice,
        ).toFixed(2);

        listArr.push(['price', el.image_base64, `${_data.symbol}${result}`]);
        listArr.push([
          `${el.quantity} X ${title}`,
          '',
          `${_data.symbol}${result}`,
        ...
        SunmiPrinter.printColumnsText(
          [`${el.quantity} X ${title}`, '', `${_data.symbol} ${result}`],
          columnWidth,
          columnAliment,
        );
        // await SunmiPrinter.printColumnsText(
      ...
      if (!!_data?.instruction && _data?.instruction != '') {
        ...
        SunmiPrinter.setFontSize(22);
        SunmiPrinter.setAlignment(1);
        SunmiPrinter.printOriginalText(`${_data?.instruction}\n`);
        SunmiPrinter.printOriginalText(`───────────────────────────────\n\n`);
      } else {
        SunmiPrinter.setFontSize(22);
        SunmiPrinter.setAlignment(1);
      ...
      SunmiPrinter.setFontSize(21);
      SunmiPrinter.printColumnsText(
        [`${strings.TOTAL}`, '', `${_data.symbol}${total_amt}`],
        columnWidth,
        columnAliment,
      );
      SunmiPrinter.printColumnsText(

...
const RoyoProducts = (props) => {
  ...
  const renderCatogry = ({ item, index }) => {
    ...
    // let imageUri = getImageUrl(item.cat_image.proxy_url, '2000/')
    const imageUrl = !!item?.cat_image?.proxy_url
    ? getImageUrl(
      item?.cat_image?.image_fit,
      item?.cat_image?.image_path,
      '200/200',
    )
    : getImageUrl(item.image.image_fit, item.image.image_path, '200/200');
    console.log(imageUrl,'jsabdfkjbdsfk');
   ...
   return <View
      ...
      {/* {console.log(item ,'mshdbfjdhsfbjdh',`${item.cat_image.proxy_url}200/200${item.cat_image.image_path}` )} */}
